// Firebase Konfiguration (Compat Version für bessere Browser-Kompatibilität)
...
class KlassenarbeitsPlaner {
    ...
    async init() {
        ...
        try {
            ...
            // Prüfe ob Firebase korrekt geladen wurde
            ...
            this.showNotification('Fehler beim Laden der App. Arbeite offline...', 'warning');
            ...
            const unsubscribe = auth.onAuthStateChanged(async (user) => {
                try {
                    if (user) {
                        ...
                        unsubscribe(); // Listener nur einmal ausführen
                        resolve();
                    } else {
    ...
    async setupFirestoreListener() {
        ...
        try {
            console.log('Setup Firestore Listener für User:', this.userId);
            const examsRef = db.collection('users').doc(this.userId).collection('exams');
    ...
    async updateExamInFirestore(examId, examData) {
        ...
        const examRef = db.collection('users').doc(this.userId).collection('exams').doc(examId);
